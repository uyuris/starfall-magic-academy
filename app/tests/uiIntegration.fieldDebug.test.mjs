...
test('shop screen is a full-bleed 深夜の学院 surface: own --shop-night-* token layer, hero money badge, two-panel board, inventory material badges, internal scroll (test-by-token)', async () => {
  ...
  assert.match(css, /\.economy-message-box\s*\{[\s\S]*position:\s*fixed[\s\S]*opacity:\s*0[\s\S]*transform:[\s\S]*\.economy-message-box\.visible\s*\{[\s\S]*opacity:\s*1/, 'economy message box stays an animated fixed overlay that becomes visible on trade actions');
