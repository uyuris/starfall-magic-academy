...
import {
  MAGIC_KNOWLEDGE_TEXTS,
  buildConversationActorContextSnapshot,
  buildMagicKnowledgeActorContext,
  renderConversationActorContext
} from '../src/llm/conversationActorContext.mjs';

const expectedMagicKnowledgeTexts = {
  light: {
...
test('MAGIC_KNOWLEDGE_TEXTS stores the twelve approved knowledge passages verbatim', () => {
  assert.deepEqual(MAGIC_KNOWLEDGE_TEXTS, expectedMagicKnowledgeTexts);
  assert.equal(Object.values(MAGIC_KNOWLEDGE_TEXTS).flatMap((tiers) => Object.values(tiers)).length, 12);
});

test('buildMagicKnowledgeActorContext gates basic and advanced knowledge by actor magic parameters', () => {
  const context = buildMagicKnowledgeActorContext({
    magic: { light: 80, dark: 10, fire: 50, water: 40, earth: 20, wind: 49 },
    abilities: {}
  });

  assert.deepEqual(
    context.sections[0].entries.map((entry) => entry.title),
    ['光・基礎', '光・応用', '火・基礎']
  );
  assert.match(context.sections[0].entries[0].body, /^光魔法の基礎。/);
  assert.match(context.sections[0].entries[1].body, /^光魔法の応用。/);
  assert.match(context.sections[0].entries[2].body, /^火魔法の基礎。/);
  assert.equal(buildMagicKnowledgeActorContext({
    magic: { light: 49, dark: 49, fire: 49, water: 49, earth: 49, wind: 49 },
    abilities: {}
  }), null);
});

test('buildConversationActorContextSnapshot injects affinity only for academy character ids', async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'magic-adv-actor-context-'));
  t.after(async () => {
    await fs.rm(root, { recursive: true, force: true });
  });

  const context = await buildConversationActorContextSnapshot({
    root,
    actor: {
      id: 'mentor_alma',
      kind: 'character',
      basePath: 'game_data/characters/mentor_alma'
    },
    profile: {
      parameters: {
        magic: { light: 50, dark: 0, fire: 0, water: 0, earth: 0, wind: 0 },
        abilities: {}
      }
    }
  });

  assert.deepEqual(context.sections.map((section) => section.title), ['系統知識']);
  assert.doesNotMatch(renderConversationActorContext(context), /主人公への好感度:/);
});

test('renderConversationActorContext fails fast on malformed actor-context sections', () => {
  assert.throws(
    () => renderConversationActorContext({ sections: [] }),
    /conversationActorContext\.sections must not be empty/
  );
  assert.throws(
    () => renderConversationActorContext({ sections: [{ title: '系統知識', entries: [] }] }),
    /conversationActorContext\.sections\[0\]\.entries must not be empty/
  );
  assert.throws(
    () => renderConversationActorContext({ sections: [{ title: '系統知識', entries: [{ title: '光・基礎', body: '' }] }] }),
    /conversationActorContext\.sections\[0\]\.entries\[0\]\.body must not be empty/
  );
  assert.throws(
    () => renderConversationActorContext({ sections: [{ title: '', entries: [{ title: '光・基礎', body: '本文' }] }] }),
    /conversationActorContext\.sections\[0\]\.title must not be empty/
  );
});
